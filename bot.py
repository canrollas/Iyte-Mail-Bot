...
examples of graphs, """
data3="""connectedness, Eulerian and Hamiltonian paths and cycles, and
...
data4="""Throughout the book I have attempted to restrict the text to basic material, using
...
asterisk."""
data5="""I have used the symbol // to indicate the end of a proof, and bold-face type is used
for definitions. The number of elements in a set S is denoted by LSI, and the empty set
is denoted by 0."""
data6="""A substantial number of changes have been made in this edition. The text has been
...
"""
lister=[data,data2,data3,data4,data5,data6]
names=["bilgehanay","canrollas","alindakabadayı","berkcantug","özlemkaba","rollasekrem"]

try:
    
    driver.find_element_by_id("username").send_keys("xxxxxxxxxx@std.iyte.edu.tr")
    driver.find_element_by_id ("password").send_keys("*************")
    driver.find_element_by_class_name("ZLoginButton").click()    
    time.sleep(3)
    counter=0
    for i,j in zip(names,lister):
        while True:
        ...
        time.sleep(1)
        driver.find_element_by_id("zv__COMPOSE-1_to_control").send_keys(i+"@std.iyte.edu.tr")
        driver.find_element_by_id("zv__COMPOSE-1_subject_control").send_keys(x)
        driver.find_element_by_id("ZmHtmlEditor1_body_ifr").send_keys(j)
        driver.find_element_by_id('zb__COMPOSE-1__SEND_left_icon').click()
        print("İsmi {} kisiye {} tane mail gönderildi".format(i,counter))
        counter=counter+1
        time.sleep(1)
